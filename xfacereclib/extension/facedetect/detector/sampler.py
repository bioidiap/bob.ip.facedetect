...
class Sampler:
  ...
  def __init__(self, patch_size = (24,20), scale_factor = math.pow(0.5, 1./2.), distance = 2, similarity_thresholds = (0.3, 0.7)):
  ...
  def add(self, image, ground_truth, number_of_positives_per_scale = None, number_of_negatives_per_scale = None):
    ...
    current_scale_power = 2.
    while True:
      # scale the image
      scale = math.pow(self.m_scale_factor, current_scale_power)
      ...
      distance = int(math.ceil(self.m_distance*scale))
...
#              facereclib.utils.debug("Rejecting negative bounding box %s  -- '%s' with similarity value %f" % (str(bb), str(gt), similarity))
  ...
  def get(self, feature_extractor, maximum_number_of_positives = None, maximum_number_of_negatives = None):
    ...
    # compute a random subset of both
    # (for testing purposes, this is quasi-random)
    last_image_index = -1
    i = 0
    # append positive examples
    for index in facereclib.utils.quasi_random_indices(len(all_positive_examples), num_pos):
      image_index, patch_index = all_positive_examples[index]
    ...
    # append positive examples
    for index in facereclib.utils.quasi_random_indices(len(all_negative_examples), num_neg):
      image_index, patch_index = all_negative_examples[index]
      # prepare for this image, if it has changed
      if image_index != last_image_index:
        last_image_index = image_index
        feature_extractor.prepare(self.m_scaled_images[image_index])
