...
def command_line_options(command_line_arguments):
  ...
  parser.add_argument('--databases', '-d', nargs="+", help = "Select the database to get the training images from; if not specified, a regular cascade will be created.")
...
def main(command_line_arguments = None):
  ...
  classifier, feature_extractor, mean, variance = detector.load(args.trained_file)
  if not args.databases:
    facereclib.utils.info("Creating regular cascade from strong classifier")
    ...
    # open database to collect validation images
